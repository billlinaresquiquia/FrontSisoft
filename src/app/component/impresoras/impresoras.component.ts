...
export class ImpresorasComponent {
  productosPorCategoriaimpresora: Product[] = [];
  ...
  ngOnInit(): void {
    ...
    this.getProductosPorCategoria(4); // Reemplaza 1 con el ID de la categoría deseada
  }

  getProductosPorCategoria(categoriaId: number) {
    this.productService.getProductsByCategory(categoriaId).subscribe(
      (data: Product[]) => {
        this.productosPorCategoriaimpresora = data;
